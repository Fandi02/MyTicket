...
namespace MyTicket.Application.Businesses.Auth.Commands
{
    ...
    public class LoginCommandHandler : IRequestHandler<LoginCommand, LoginResponse>
    {
        ...
        public LoginCommandHandler(IMyTicketDbContext dbContext)
        {
        ...
        public async Task<LoginResponse> Handle(LoginCommand request, CancellationToken cancellationToken)
        {
            ...
            var responseUser = await _dbContext.Users.Include(x => x.UserPasswords).FirstOrDefaultAsync(x => x.Email == request.Email && x.IsDeleted == false);
            ...
            {
                if (responseUser.Role != UserRoleEnum.Admin)
                {
            ...
            {
                if (responseUser.Role != UserRoleEnum.User)
                {
                    throw new BadRequestException("Please check your account role and try again");
                } 
