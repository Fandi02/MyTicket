...
public class GetListOrderByUserIdQuery : BasePagination, IRequest<IEnumerable<GetOrderQueryResponse>>
{
...
public class GetListOrderByUserIdQueryHandler : IRequestHandler<GetListOrderByUserIdQuery, IEnumerable<GetOrderQueryResponse>>
{
    private readonly IContext _context;
    private readonly IMyTicketDbContext _dbContext;
    public GetListOrderByUserIdQueryHandler(IContext context, IMyTicketDbContext dbContext)
    {
        _context = context;
        _dbContext = dbContext;
    }

    public async Task<IEnumerable<GetOrderQueryResponse>> Handle(GetListOrderByUserIdQuery request, CancellationToken cancellationToken)
    {
        if (request.UserId == Guid.Empty)
            throw new BadRequestException("Request is null");
            
        var getData = await _dbContext.OrderTickets.Include(x => x.Event).Where(x => x.UserId == request.UserId && x.IsPaid == false && x.IsDeleted == false).ToListAsync(cancellationToken);
