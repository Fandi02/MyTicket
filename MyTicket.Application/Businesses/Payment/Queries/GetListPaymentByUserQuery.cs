...
public class GetListPaymentByUserQuery : BasePagination, IRequest<IEnumerable<GetPaymentQueryResponse>>
{
...
public class GetListPaymentByUserQueryHandler : IRequestHandler<GetListPaymentByUserQuery, IEnumerable<GetPaymentQueryResponse>>
{
    private readonly IContext _context;
    private readonly IMyTicketDbContext _dbContext;
    public GetListPaymentByUserQueryHandler(IContext context, IMyTicketDbContext dbContext)
    {
        _context = context;
        _dbContext = dbContext;
    }

    public async Task<IEnumerable<GetPaymentQueryResponse>> Handle(GetListPaymentByUserQuery request, CancellationToken cancellationToken)
    {  
        if (request.UserId == Guid.Empty)
            throw new BadRequestException("Request is null");
            
        var getData = await _dbContext.Payments.Include(x => x.User).Include(x => x.OrderTicket.Event).Where(x => x.UserId == request.UserId && x.IsDeleted == false).ToListAsync(cancellationToken);
