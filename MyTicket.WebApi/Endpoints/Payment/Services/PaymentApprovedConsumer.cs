...
public class PaymentApprovedConsumer : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        channel.QueueDeclare(queue: "payment-approved", durable: false, exclusive: false, autoDelete: true, arguments: null);
        ...
        consumer.Received += async (model, ea) =>
        {
            using (var scope = _serviceProvider.CreateScope())  // Membuat scope baru
            {
                ...
                if (rejectedPaymentRequest != null && rejectedPaymentRequest.Email != null)
                {
                    ...
                    var bodyText = $"Hello {rejectedPaymentRequest.FullName}, event {rejectedPaymentRequest.EventName} - ticket ({rejectedPaymentRequest.TicketNumber}) has been approved!";
