...
public class GetListOrderByUserIdController : BaseEndpoint<GetListOrderRequest, GetListOrderResponse>
{
    private readonly IMediator _mediator;
    public GetListOrderByUserIdController(IMediator mediator)
    {
        _mediator = mediator;
    }

    [HttpGet("get-by-user-only")]
    [Authorize]
    [SwaggerOperation(
    ...
    public override async Task<ActionResult<GetListOrderResponse>> HandleAsync([FromQuery] GetListOrderRequest request, CancellationToken cancellationToken = default)
    {
        try {
            ...
            if (role != UserRoleEnum.User.ToString())
                throw new BadRequestException("Only user can get list order ticket");

            var mediator = _mediator;
            if (mediator is null)
            {
                var logger = HttpContext.RequestServices.GetRequiredService<ILogger<GetListOrderByUserIdController>>();
            ...
            var result = await _mediator.Send(new GetListOrderByUserIdQuery 
                                                { 
